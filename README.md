...
A self-hosted, cloud-deployed personal tool with two modes: **build**, a
raw-idea-to-precise-prompt enhancer, and **roast**, a conversational
stress-test for a plan before you build it.

## Why this exists

1. **Save tokens on plan writing and research.** The expensive part of working with AI builders is re-explaining your taste, stack, and standards every time. Here that lives in an editable profile that gets injected into every enhancement and every roast.
...
3. **A model for the moment.** Four Claude tiers (Haiku 4.5 → Sonnet 5 → Opus 4.8 → Fable 5) are one click apart — cheap and fast for a quick pass, top-tier when the idea deserves it. Cost per turn ranges from a fraction of a cent to a few cents.
...
**Roast mode**
```
paste a plan ──► AI stress-tests it in character (blunt, funny, concrete)
                  ──► you respond, it presses on unresolved points,
                      drops the ones you've adequately answered
                      ──► keeps going until you're done
```

- **Engine:** Claude API, four selectable tiers (haiku/sonnet/opus/fable) — same picker, both modes.
- **Profile:** one editable markdown document (design language, copy voice, implementation defaults, standards) injected into both modes.
